...
experiments. Start with Notebooks/01_dataset_understanding.ipynb.
...
Requires Python 3.10 or newer and the packages in requirements.txt. Runs on
Google Colab as well as a local environment.
"""

import sys

# 3.10 is the real floor: it is the lowest version every range in
# requirements.txt resolves on, and nothing here uses 3.11+ syntax. Keeping the
# guard at the true minimum matters because Colab pins its own interpreter --
# refusing to run on a version that works would block a teammate for no reason.
